...
describe("Search", () => {
  ...
  it.each`
    ...
    ${"v3 SearchField"} | ${SearchField}
  `(
    "$Name should display the clear button icon only if text is present",
    ({ Component }) => {
  ...
  it.each`
    ...
    ${"v3 SearchField"} | ${SearchField}
  `(
    "$Name submits the textfield value when enter is pressed",
    ({ Component }) => {
      let tree = renderComponent(Component, {
  ...
  it("SearchField doesn't show clear button if isReadOnly is true", () => {
    let tree = renderComponent(SearchField, {
      isReadOnly: true,
      value: "puppy",
    });
