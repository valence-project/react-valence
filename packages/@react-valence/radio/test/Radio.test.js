...
function renderRadioGroupNoLabel(
  ComponentGroup,
  Component,
  groupProps,
  radioProps
...
expect.extend({
  checkRadioIndex(received, tabIndices, i) {
    ...
    if (index !== -1) {
      return {
        message: () =>
          `expected radio index configuration "radio${
            i + 1
          }Focused": got (${received.map((radio) =>
...
describe("Radios", function () {
  ...
  `(
    ...
    function ({ ComponentGroup, Component, groupProps, radioProps }) {
      let { getAllByRole } = renderRadioGroup(
        ComponentGroup,
        Component,
        groupProps,
        radioProps
  ...
  it("v3 RadioGroup supports labeling", () => {
    let { getByRole } = renderRadioGroupNoLabel(
      RadioGroup,
      Radio,
      { label: "Favorite Pet" },
      {}
  ...
  it('v3 RadioGroup sets aria-invalid when validationState="invalid"', () => {
    let { getByRole } = renderRadioGroup(
      RadioGroup,
      Radio,
      { label: "Favorite Pet", validationState: "invalid" },
      []
    );
    let radioGroup = getByRole("radiogroup");
  ...
  describe("Radio group supports roving tabIndex ", function () {
    ...
    `(
      "$Name default keyboard navigation with wrapping",
      async ({ props, orders }) => {
        let { getByRole, getAllByRole } = render(
    ...
    `(
      "$Name skips multiple disabled radios",
      function ({ Name, props, disabledKeys, orders }) {
        let tree = render(
          <Provider theme={theme} locale={props.locale}>
            <RadioGroup aria-label="favorite pet" orientation="horizontal">
