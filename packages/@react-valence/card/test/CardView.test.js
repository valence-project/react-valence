...
describe("CardView", function () {
  ...
  `("$Name CardView supports falsy ids", function ({ layout }) {
    ...
    for (let row of rowgroups) {
      ...
      expect(image).toHaveAttribute("src");
      expect(
        within(cell).getByText("long description", { exact: false })
      ).toBeTruthy();
      expect(within(cell).getByText("PNG")).toBeTruthy();
    }
  });
  ...
  describe("Grid layout and some shared gallery layout behavior", function () {
    ...
    `(
      ...
      function ({ cardOrientation }) {
        let tree = render(
          <DynamicCardView cardOrientation={cardOrientation} />
        );
        act(() => {
          jest.runAllTimers();
        });
        ...
        for (let [index, div] of wrappers.entries()) {
          ...
          if (!expectedWidth) {
            ...
            if (!expectedSpacing) {
              let cardsInRow = Math.floor(
                (mockWidth - 24 * 2) / parseInt(expectedWidth, 10)
              );
              if (cardsInRow > 1) {
                expectedSpacing =
                  (mockWidth -
                    24 * 2 -
                    cardsInRow * parseInt(expectedWidth, 10)) /
                  (cardsInRow - 1);
              } else {
                expectedSpacing = 0;
              }
            ...
            if (currentTop === div.style.top) {
              currentLeft = `${
                parseInt(currentLeft, 10) +
                parseInt(expectedWidth, 10) +
                expectedSpacing
              }px`;
            } else {
              // default space between the two cards vertically is 18px
              currentTop = `${
                parseInt(currentTop, 10) + parseInt(expectedHeight, 10) + 18
              }px`;
              currentLeft = "24px";
    ...
    describe("keyboard nav", function () {
      it("should move focus via Arrow Down", function () {
        ...
        let expectedTop = `${
          parseInt(cardStyles.top, 10) + parseInt(cardStyles.height, 10) + 18
        }px`;
      ...
      it.each`
        ...
        ${"Gallery layout"} | ${GalleryLayout}
      `(
        "$Name CardView should move focus via Arrow Left",
        function ({ Name, layout }) {
          ...
          expectedLeft = `${
            parseInt(expectedLeft, 10) -
            parseInt(cardStyles.width, 10) -
            horizontalSpacing
          }px`;
          expect(cardStyles.top).toEqual(expectedTop);
          expect(cardStyles.left).toEqual(expectedLeft);
      ...
      it.each`
        ...
        ${"Gallery layout"} | ${GalleryLayout}
      `(
        "$Name CardView should move focus via Arrow Left (RTL)",
        function ({ Name, layout }) {
      ...
      it.each`
        ...
        ${"Gallery layout"} | ${GalleryLayout}
      `(
        "$Name CardView should move focus via Arrow Right",
        function ({ Name, layout }) {
          let tree = render(<DynamicCardView layout={layout} />);
          act(() => {
            jest.runAllTimers();
      ...
      it.each`
        ...
        ${"Gallery layout"} | ${GalleryLayout}
      `(
        "$Name CardView should move focus via Arrow Right (RTL)",
        function ({ Name, layout }) {
          let tree = render(<DynamicCardView locale="ar-AE" layout={layout} />);
          act(() => {
            jest.runAllTimers();
      ...
      it("should move focus via Page Up", function () {
        ...
        let numCardsInPage = Math.floor(
          mockHeight / (parseInt(cardStyles.height, 10) + 18)
        );
        let expectedTop = `${
          parseInt(cardStyles.top, 10) -
          numCardsInPage * (parseInt(cardStyles.height, 10) + 18)
        }px`;
  ...
  describe("Gallery layout", function () {
    it("renders cards in rows with constrained heights and variable widths", function () {
      ...
      for (let [index, div] of wrappers.entries()) {
        ...
        if (!expectedHeight) {
          ...
          expectedLeft = `${
            parseInt(div.style.left, 10) + parseInt(div.style.width, 10) + 18
          }px`;
        } else {
          if (currentTop === div.style.top) {
    ...
    describe("keyboard nav", function () {
      it("should move focus via Arrow Down", function () {
        ...
        expect(document.activeElement).toBe(cards[0]);
        expect(
          within(document.activeElement).getByText("Title 1")
        ).toBeTruthy();
        ...
        expect(cardStyles.top).toEqual(expectedTop);
        expect(
          within(document.activeElement).getByText("Title 4")
        ).toBeTruthy();
      });
      ...
      it("should move focus via Arrow Up", function () {
        ...
        cardStyles = getCardStyles(document.activeElement);
        expectedTop = `${
          parseInt(expectedTop, 10) - parseInt(cardStyles.height, 10) - 18
        }px`;
        expect(cardStyles.top).toEqual(expectedTop);
        expect(
          within(document.activeElement).getByText("Title 1")
      ...
      it("should move focus via Page Up", function () {
        ...
        let cardStyles = getCardStyles(document.activeElement);
        let numCardsInPage = Math.floor(
          mockHeight / (parseInt(cardStyles.height, 10) + 32)
        );
  ...
  describe("Waterfall layout", function () {
    ...
    `(
      ...
      function ({ items }) {
        let tree = render(
          <DynamicCardView layout={WaterfallLayout} items={items} />
        );
        act(() => {
          jest.runAllTimers();
        });
        ...
        for (let [index, div] of wrappers.entries()) {
          ...
          if (!expectedWidth) {
            ...
            let expectedHeight = `${
              parseInt(div.style.top, 10) + parseInt(div.style.height, 10) + 18
            }px`;
    ...
    describe("keyboard nav", function () {
      it("should move focus via Arrow Down", function () {
        ...
        expect(cardStyles.left).toEqual(expectedLeft);
        expect(
          within(document.activeElement).getByText("Title 3")
        ).toBeTruthy();
      });
      ...
      it("should move focus via Arrow Left", function () {
        ...
        cardStyles = getCardStyles(document.activeElement);
        expectedLeft = `${
          parseInt(expectedLeft, 10) - parseInt(cardStyles.width, 10) - 18
        }px`;
        expect(cardStyles.top).toEqual(expectedTop);
        expect(cardStyles.left).toEqual(expectedLeft);
      });
      ...
      it("should move focus via Arrow Left (RTL)", function () {
        let tree = render(
          <DynamicCardView locale="ar-AE" layout={WaterfallLayout} />
        );
        act(() => {
          jest.runAllTimers();
        });
        ...
        cardStyles = getCardStyles(document.activeElement);
        expectedRight = `${
          parseInt(expectedRight, 10) + parseInt(cardStyles.width, 10) + 18
        }px`;
        expect(cardStyles.top).toEqual(expectedTop);
        expect(cardStyles.right).toEqual(expectedRight);
      });
  ...
  describe("common keyboard nav", function () {
    ...
    it.each`
      ...
      ${"Waterfall layout"} | ${WaterfallLayout}
    `(
      "$Name CardView should move focus via type to select",
      function ({ layout }) {
        let tree = render(<DynamicCardView layout={layout} />);
        act(() => {
          jest.runAllTimers();
        ...
        expect(
          within(document.activeElement).getByText("Title 12")
        ).toBeTruthy();
      }
    );
  });
  ...
  describe("selection", function () {
    ...
    it("CardView should support selectedKeys", function () {
      let tree = render(
        <DynamicCardView selectedKeys={["Title 1", "Title 2"]} />
      );
      act(() => {
        jest.runAllTimers();
      });
      ...
      expect(within(cards[0]).getByRole("checkbox")).toHaveAttribute(
        "aria-checked",
        "true"
      );
      expect(within(cards[1]).getByRole("checkbox")).toHaveAttribute(
        "aria-checked",
        "true"
      );
    });
    ...
    it("CardView should support disabledKeys", function () {
      ...
      expect(within(cards[0]).getByRole("checkbox")).not.toHaveAttribute(
        "aria-checked",
        "true"
      );
    ...
    it("CardView should support multiple selection", function () {
      ...
      expect(within(cards[2]).getByRole("checkbox")).toHaveAttribute(
        "aria-checked",
        "true"
      );
  ...
  describe("loading", function () {
    it.each`
      ...
      ${"Waterfall layout"} | ${WaterfallLayout}
    `(
      "$Name CardView should render a loading spinner when loading",
      function ({ layout }) {
        let tree = render(
          <DynamicCardView layout={layout} items={[]} loadingState="loading" />
        );
        act(() => {
          jest.runAllTimers();
        });
    ...
    `(
      ...
      function ({ layout }) {
        let tree = render(
          <DynamicCardView layout={layout} loadingState="loadingMore" />
        );
        act(() => {
          jest.runAllTimers();
        });
    ...
    `(
      ...
      function ({ layout }) {
        ...
        let tree = render(
          <DynamicCardView layout={layout} onLoadMore={onLoadMore} />
        );
    ...
    `(
      ...
      function ({ layout }) {
        let tree = render(
          <DynamicCardView layout={layout} loadingState="filtering" />
        );
        act(() => {
          jest.runAllTimers();
        });
  ...
  describe("emptyState", function () {
    it.each`
      ...
      ${"Waterfall layout"} | ${WaterfallLayout}
    `(
      "$Name CardView should render empty state when there are no items",
      function ({ layout }) {
