...
// @types-valence
import { Node, Orientation } from "@types-valence/shared";
...
export class GridLayout<T> extends BaseLayout<T> {
  ...
  constructor(options: GridLayoutOptions = {}) {
    ...
    this.minItemSize =
      options.minItemSize ||
      DEFAULT_OPTIONS[cardSize].minItemSize[this.cardOrientation];
    this.maxItemSize =
      options.maxItemSize || DEFAULT_OPTIONS[cardSize].maxItemSize;
    this.margin =
      options.margin != null
        ? options.margin
        : DEFAULT_OPTIONS[cardSize].margin;
    this.minSpace = options.minSpace || DEFAULT_OPTIONS[cardSize].minSpace;
    this.maxColumns =
      options.maxColumns || DEFAULT_OPTIONS[cardSize].maxColumns;
    this.itemPadding =
      options.itemPadding != null
        ? options.itemPadding
        : DEFAULT_OPTIONS[cardSize].itemPadding[this.cardOrientation][
            this.scale
          ];
    this.itemSize = null;
    this.numColumns = 0;
  ...
  buildCollection() {
    ...
    let horizontalItemPadding =
      this.cardOrientation === "horizontal" ? this.itemPadding : 0;
    let verticalItemPadding =
      this.cardOrientation === "vertical" ? this.itemPadding : 0;
    let minCardWidth = this.minItemSize.width + horizontalItemPadding;

    // Compute the number of rows and columns needed to display the content
    let availableWidth = visibleWidth - this.margin * 2;
    let columns = Math.floor(
      (availableWidth + this.minSpace.width) /
        (minCardWidth + this.minSpace.width)
    );
    ...
    let width =
      availableWidth - this.minSpace.width * Math.max(0, this.numColumns - 1);

    // Compute the item width based on the space available
    let itemWidth = Math.floor(width / this.numColumns);
    itemWidth = Math.max(
      minCardWidth,
      Math.min(this.maxItemSize.width, itemWidth)
    );
    // Compute the item height, which is proportional to the item width
    let t = (itemWidth - minCardWidth) / minCardWidth;
    let itemHeight = Math.floor(
      this.minItemSize.height + this.minItemSize.height * t
    );
    itemHeight =
      Math.max(
        this.minItemSize.height,
        Math.min(this.maxItemSize.height, itemHeight)
      ) + verticalItemPadding;
    ...
    this.horizontalSpacing =
      this.numColumns < 2
        ? 0
        : Math.floor(
            (availableWidth - this.numColumns * this.itemSize.width) /
              (this.numColumns - 1)
          );
  ...
  buildChild(node: Node<T>, y: number, index: number): LayoutInfo {
    ...
    let x =
      this.margin + column * (this.itemSize.width + this.horizontalSpacing);
  ...
  getKeyBelow(key: Key) {
    ...
    let indexRowBelow;
    let index = this.collection.rows.findIndex(
      (card) => card.key === parentRowKey
    );
    if (index !== -1) {
