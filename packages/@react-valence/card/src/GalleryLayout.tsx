...
export class GalleryLayout<T> extends BaseLayout<T> {
  ...
  constructor(options: GalleryLayoutOptions = {}) {
    ...
    let cardSize = "L";
    this.idealRowHeight =
      options.idealRowHeight || DEFAULT_OPTIONS[cardSize].idealRowHeight;
    this.itemSpacing =
      options.itemSpacing || DEFAULT_OPTIONS[cardSize].itemSpacing;
    this.itemPadding =
      options.itemPadding != null
        ? options.itemPadding
        : DEFAULT_OPTIONS[cardSize].itemPadding[this.scale];
    this.minItemSize =
      options.minItemSize || DEFAULT_OPTIONS[cardSize].minItemSize;
  ...
  buildCollection() {
    ...
    if (availableWidth > 0) {
      ...
      for (let row of partition) {
        ...
        let bestRowHeight =
          (availableWidth - (row.length - 1) * this.itemSpacing.width) /
          totalWeight;
        ...
        if (
          row === partition[partition.length - 1] &&
          bestRowHeight > this.idealRowHeight * 2
        ) {
