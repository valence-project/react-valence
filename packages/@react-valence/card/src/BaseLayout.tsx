...
// @types-valence
import { Direction, KeyboardDelegate, Node } from "@types-valence/shared";
...
export class BaseLayout<T> extends Layout<Node<T>> implements KeyboardDelegate {
  ...
  buildCollection(
    invalidationContext?: InvalidationContext<Node<T>, unknown>
  ) {}
  ...
  _findClosestLayoutInfo(target: Rect, rect: Rect) {
    ...
    for (let cur of layoutInfos) {
      if (cur.type === "item") {
        ...
        let dist =
          Math.pow(targetMidX - curMidX, 2) + Math.pow(targetMidY - curMidY, 2);
  ...
  getKeyBelow(key: Key) {
    ...
    let closestRow = this.collection.getItem(
      this._findClosest(layoutInfo.rect, rect)?.key
    );
    ...
    if (layoutInfo) {
      ...
      if (layoutInfo && layoutInfo.rect.y > pageY) {
        while (layoutInfo && layoutInfo.rect.y > pageY) {
          let childKey = this.collection.getItem(layoutInfo.key).childNodes[0]
            .key;
          let keyAbove = this.collection.getItem(
            this.getKeyAbove(childKey)
          )?.parentKey;
          layoutInfo = this.getLayoutInfo(keyAbove);
        }
      }
    ...
    if (layoutInfo) {
      ...
      if (layoutInfo && layoutInfo.rect.y < pageY) {
        while (layoutInfo && layoutInfo.rect.y < pageY) {
          let childKey = this.collection.getItem(layoutInfo.key).childNodes[0]
            .key;
          let keyBelow = this.collection.getItem(
            this.getKeyBelow(childKey)
          )?.parentKey;
          layoutInfo = this.getLayoutInfo(keyBelow);
        }
      }
