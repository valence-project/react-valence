...
// @types-valence
import { KeyboardDelegate, Node } from "@types-valence/shared";
...
export class WaterfallLayout<T>
  ...
  buildCollection(invalidationContext: InvalidationContext<Node<T>, unknown>) {
    ...
    let columns = Math.floor(
      (availableWidth + this.minSpace.width) /
        (this.minItemSize.width + this.minSpace.width)
    );
    ...
    let horizontalSpacing = Math.round(
      (availableWidth - this.numColumns * itemWidth) /
        Math.max(1, this.numColumns - 1)
    );
    ...
    for (let node of this.collection) {
      ...
      if (oldLayoutInfo) {
        ...
        estimatedSize =
          invalidationContext.sizeChanged || oldLayoutInfo.estimatedSize;
        ...
        height = Math.max(
          ...
          Math.min(this.maxItemSize.height, scaledHeight)
        );
      } else {
        height = itemWidth;
      }
    ...
    let maxHeight =
      Math.max.apply(Math, columnHeights) - this.minSpace.height + this.margin;
