...
describe("Slider", function () {
  ...
  it("supports showValueLabel: false", function () {
    let { getByRole, queryByRole } = render(
      <Slider label="The Label" showValueLabel={false} />
    );
    let group = getByRole("group");
  ...
  describe("keyboard interactions", () => {
    ...
    `(
      "$Name moves the slider in the correct direction",
    ...
    it.each`
      ...
      ${"(page up/down, rtl)"}      | ${{ locale: "ar-AE" }} | ${[{ left: press.PageUp, result: +20 }, { left: press.PageDown, result: -20 }]}
    `(
      "$Name sets page size to a multiple of step",
      function ({ props, commands }) {
        let tree = render(
          <Provider theme={theme} {...props}>
            <Slider
  ...
  describe("mouse interactions", () => {
    ...
    it("can click on track to move handle", () => {
      ...
      // @ts-ignore
      let [leftTrack, rightTrack] = [...thumb.parentElement.children].filter(
        (c) => c !== thumb
      );
  ...
  describe("touch interactions", () => {
    ...
    it("doesn't jump to second touch on track while already dragging", () => {
      ...
      let thumb = slider.parentElement.parentElement;
      // @ts-ignore
      let [, rightTrack] = [...thumb.parentElement.children].filter(
        (c) => c !== thumb
      );

      fireEvent.touchStart(thumb, {
        changedTouches: [{ identifier: 1, clientX: 50, pageX: 50 }],
      });
      expect(onChangeSpy).toHaveBeenCalledTimes(0);

      fireEvent.touchStart(rightTrack, {
        changedTouches: [{ identifier: 2, clientX: 60, pageX: 60 }],
      });
      fireEvent.touchMove(rightTrack, {
        changedTouches: [{ identifier: 2, clientX: 70, pageX: 70 }],
      });
      fireEvent.touchEnd(rightTrack, {
        changedTouches: [{ identifier: 2, clientX: 70, pageX: 70 }],
      });
      expect(onChangeSpy).toHaveBeenCalledTimes(0);

      fireEvent.touchMove(thumb, {
        changedTouches: [{ identifier: 1, clientX: 30, pageX: 30 }],
      });
      expect(onChangeSpy).toHaveBeenCalledTimes(1);
