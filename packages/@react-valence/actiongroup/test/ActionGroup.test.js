...
expect.extend({
  checkButtonIndex(received, tabIndices, i) {
    ...
    if (index !== -1) {
      return {
        message: () =>
          `expected button index configuration "button${
            i + 1
          }Focused": got (${received.map((button) =>
            button.getAttribute("tabIndex")
          )}) but expected ${tabIndices}`,
        pass: false,
      };
    } else {
      return {
...
describe("ActionGroup", function () {
  ...
  `(
    "$Name skips disabled keys",
    function ({ Name, props, disabledKeys, orders }) {
      let tree = render(
        <Provider theme={theme} locale={props.locale}>
  ...
  `(
    "$Name skips multiple disabled keys",
    function ({ Name, props, disabledKeys, orders }) {
      let tree = render(
        <Provider theme={theme} locale={props.locale}>
          <ActionGroup disabledKeys={disabledKeys}>
  ...
  it("ActionGroup handles multiple selection", function () {
    let { getByRole, getAllByRole } = renderComponent({
  ...
  it("ActionGroup should pass className, role and tabIndex", function () {
    ...
    expect(button1).toHaveAttribute(
      "class",
      expect.not.stringContaining("test-class")
    );
  ...
  describe('overflowMode="collapse"', function () {
    ...
    it("menu items should be disabled for items listed in disabledKeys", function () {
      ...
      expect(
        within(actionGroup).getByRole("button", { name: "One" })
      ).toBeVisible();
  ...
  describe("buttonLabelBehavior", function () {
    it('should show the text in a tooltip with buttonLabelBehavior="hide"', function () {
      ...
      let text = document.getElementById(
        buttons[0].getAttribute("aria-labelledby")
      );
