...
import ThreeDotsVertical from "@valence-icons/ui/MoreFill";

// @valence-styles
import buttonStyles from "@valence-styles/components/button/vars.module.scss";
import styles from "@valence-styles/components/actiongroup/vars.module.scss";

function ActionGroup<T extends object>(props: ValenceActionGroupProps<T>, ref: DOMRef<HTMLDivElement>) {
  ...
  let updateOverflow = useCallback(() => {
    ...
    if (orientation === 'vertical' && selectionMode !== 'none') {
      // Collapsing vertical action groups with selection is currently unsupported by Valence.
      return;
    }
      ...
      <div
        ...
        className={
          classNames(
            ...
            'ActionGroup',
            {
              'ActionGroup--quiet': isQuiet,
              'ActionGroup--vertical': isVertical,
              'ActionGroup--compact': density === 'compact',
              'ActionGroup--justified': isJustified && !isMeasuring,
              'ActionGroup--overflowCollapse': overflowMode === 'collapse'
            },
 ...
 */
const _ActionGroup = forwardRef(ActionGroup) as <T>(props: ValenceActionGroupProps<T> & { ref?: DOMRef<HTMLDivElement> }) => ReactElement;
            ...
            UNSAFE_className={
              classNames(
                ...
                'ActionGroup-item',
                {
                  ...
                  'ActionGroup-item--iconOnly': hideButtonText,
                  'ActionGroup-item--isDisabled': isDisabled
                },
                classNames(
                  buttonStyles,
                  {
...
function ActionGroupMenu<T>({ state, isDisabled, isEmphasized, staticColor, items, onAction, summaryIcon, hideButtonText, isOnlyItem, orientation, ...otherProps }: ActionGroupMenuProps<T>) {
  ...
  if (summaryIcon) {
    ...
    summaryIcon = (
      <>
        <ChevronDown UNSAFE_className={classNames(styles, 'ActionGroup-menu-chevron')} />
        <span className={classNames(styles, 'ActionGroup-menu-contents', { 'ActionGroup-item--iconOnly': iconOnly })}>
          {summaryIcon}
        </span>
      </>
    );
      ...
      <SlotProvider
        slots={{
          text: {
            id: hideButtonText ? textId : null,
            isHidden: hideButtonText,
            UNSAFE_className: classNames(styles, 'ActionGroup-menu-text')
          }
        }}>
        <PressResponder {...mergeProps(buttonProps, hoverProps)}>
          <ActionButton
            ...
            isDisabled={isDisabled}
            staticColor={staticColor}>
            {summaryIcon ? <ThreeDotsVertical /> : null}
          </ActionButton>
        </PressResponder>
      </SlotProvider>
      <Menu
