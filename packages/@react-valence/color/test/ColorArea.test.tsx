...
import {
  XBlueYGreen as DefaultColorArea,
  XSaturationYBrightness,
  XSaturationYLightness,
} from "../stories/ColorArea.stories";
    ...
    describe("attributes", () => {
      ...
      describe("labelling", () => {
        it("should support a custom aria-label", () => {
          ...
          expect(colorAreaGroup).toHaveAttribute(
            "aria-label",
            "Color hue, Color picker"
          );
      ...
      describe("keyboard events", () => {
        ...
        `("$Name", ({ props, actions: { forward, backward }, result }) => {
          ...
          expect(xSlider.getAttribute("aria-valuetext")).toBe(
            [
              `${props.defaultValue.getChannelName(
                "red",
                "en-US"
              )}: ${props.defaultValue.formatChannelValue("red", "en-US")}`,
              `${props.defaultValue.getChannelName(
                "green",
                "en-US"
              )}: ${props.defaultValue.formatChannelValue("green", "en-US")}`,
            ].join(", ")
          );
          ...
          expect(xSlider.getAttribute("aria-valuetext")).toBe(
            `${result.getChannelName(
              "red",
              "en-US"
            )}: ${result.formatChannelValue("red", "en-US")}`
          );
          ...
          expect(
            document.activeElement === xSlider ? ySlider : xSlider
        ...
        `("$Name RTL", ({ props, actions: { forward, backward }, result }) => {
          ...
          expect(xSlider.getAttribute("aria-valuetext")).toBe(
            [
              `${props.defaultValue.getChannelName(
                "red",
                "ar-AE"
              )}: ${props.defaultValue.formatChannelValue("red", "ar-AE")}`,
              `${props.defaultValue.getChannelName(
                "green",
                "ar-AE"
              )}: ${props.defaultValue.formatChannelValue("green", "ar-AE")}`,
            ].join(", ")
          );
          expect(ySlider.getAttribute("aria-valuetext")).toBe(
            [
          ...
          expect(xSlider.getAttribute("aria-valuetext")).toBe(
            `${result.getChannelName(
              "red",
              "ar-AE"
            )}: ${result.formatChannelValue("red", "ar-AE")}`
          );
  ...
  describe("full implementation controlled", () => {
    it("sets input props rgb", () => {
      let { getAllByRole, getByLabelText } = render(
        <DefaultColorArea {...DefaultColorArea.args} />
      );
      let sliders = getAllByRole("slider");

      expect(sliders.length).toBe(2);
      let [xSlider, ySlider, zSlider] = getAllByRole("slider", {
        hidden: true,
      });
