...
function Example() {
  ...
  return (
    <Flex gap="size-150" wrap>
      <ColorField label="Primary Color (Uncontrolled)" defaultValue="#e21" />
...
function Example() {
  ...
  let isValid = React.useMemo(() => {
    return (
      value && value.red && value.red > value.green && value.red > value.blue
    );
  }, [value]);
...
function Example() {
  ...
  return (
    <ColorField
      ...
      errorMessage={
        value === null ? "Empty input not allowed." : "Not a red color."
      }
    />
  );
}
