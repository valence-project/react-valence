...
describe("ListBox", function () {
  ...
  it("renders properly", function () {
    ...
    expect(items.length).toBe(
      withSection.reduce((acc, curr) => acc + curr.children.length, 0)
    );
    let i = 1;
  ...
  it("renders with falsy id", function () {
    ...
    for (let [i, section] of sections.entries()) {
      ...
      for (let [j, item] of items.entries()) {
        expect(
          within(item).getByText(itemsWithFalsyId[i].children[j].name)
        ).toBeTruthy();
      }
    }
  });
  ...
  describe("supports type to select", function () {
    ...
    it("supports item selection using the Spacebar after search times out", function () {
      ...
      let checkmark = within(document.activeElement).queryByRole("img", {
        hidden: true,
      });
      ...
      checkmark = within(document.activeElement).getByRole("img", {
        hidden: true,
      });
      expect(checkmark).toBeTruthy();
  ...
  describe("async loading", function () {
    ...
    it("should fire onLoadMore if there aren't enough items to fill the ListBox ", async function () {
      ...
      expect(onLoadMore).toHaveBeenCalledTimes(
        parseInt(React.version, 10) >= 18 ? 3 : 4
      );
    });
  });
});
