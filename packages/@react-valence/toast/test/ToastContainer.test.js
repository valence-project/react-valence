  ...
  it("get position from provider", async () => {
    let { getByTestId } = renderComponent(
      <Provider toastPlacement="top left" theme={{ light: {}, medium: {} }}>
        <ToastContainer toasts={[]} data-testid="testId1">
          Toast
        </ToastContainer>
      </Provider>
    );
    ...
    expect(
      className.includes("react-spectrum-ToastContainer--top")
    ).toBeTruthy();
    expect(
      className.includes("react-spectrum-ToastContainer--left")
    ).toBeTruthy();
  });

  it("removes a toast via timeout", async () => {
    let { getByRole, queryByRole } = renderComponent(
      <RenderToastButton timeout={1} />
    );
    let button = getByRole("button");
