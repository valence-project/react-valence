...
describe("DatePickerBase", function () {
  describe("basics", function () {
    ...
    `(
      "$Name should render a default datepicker",
      ({ Component, numSegments }) => {
    ...
    it.each`
      ...
      ${"DateRangePicker"} | ${DateRangePicker}
    `(
      '$Name should set aria-invalid when validationState="invalid"',
      ({ Component }) => {
        let { getAllByRole } = render(
          <Component label="Date" validationState="invalid" />
        );
    ...
    it.each`
      ...
      ${"DateRangePicker"} | ${DateRangePicker}
    `(
      "$Name should set aria-readonly on non-editable segments",
      ({ Component }) => {
        let { getAllByTestId } = render(
          <Component
            label="Date"
            placeholderValue={parseZonedDateTime(
    ...
    `(
      "$Name should focus selected date over placeholderValue",
      ({ Component, props }) => {
        let { getByRole } = render(
          <Component
            label="Date"
  ...
  describe("calendar popover", function () {
    it.each`
      ...
      ${"DateRangePicker"} | ${DateRangePicker}
    `(
      "$Name should open a calendar popover when clicking the button",
      ({ Component }) => {
        let { getAllByRole } = render(
    ...
    `(
      ...
      ({ Component, props }) => {
        ...
        let cell = within(grid).getAllByLabelText("selected", {
          exact: false,
        })[0];
        let description = cell
          ...
          .join(" ");
        expect(
          description.startsWith("Selected dates cannot include weekends.")
        ).toBe(true);
      }
    );
  });
  ...
  describe("focus management", function () {
    ...
    it.each`
      ...
      ${"DateRangePicker"} | ${DateRangePicker}
    `(
      "$Name should support arrow keys to move between segments",
      ({ Component }) => {
        let { getAllByRole } = render(<Component label="Date" />);
    ...
    it.each`
      ...
      ${"DateRangePicker"} | ${DateRangePicker}
    `(
      "$Name should support arrow keys to move between segments in an RTL locale",
      ({ Component }) => {
        let { getAllByRole } = render(
    ...
    it.each`
      ...
      ${"DateRangePicker"} | ${DateRangePicker}
    `(
      "$Name should focus the first segment by default if autoFocus is set",
      ({ Component }) => {
  ...
  describe("validation", function () {
    it.each`
      ...
      ${"DateRangePicker"} | ${DateRangePicker}
    `(
      '$Name should display an error icon when validationState="invalid"',
      ({ Component }) => {
    ...
    it.each`
      ...
      ${"DateRangePicker"} | ${DateRangePicker}
    `(
      '$Name should display an checkmark icon when validationState="valid"',
      ({ Component }) => {
        let { getByTestId } = render(
          <Component label="Date" validationState="valid" />
        );
