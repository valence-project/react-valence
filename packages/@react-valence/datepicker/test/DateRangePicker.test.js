...
describe("DateRangePicker", function () {
    ...
    it('should render a DateRangePicker granularity="second"', function () {
      let { getAllByRole } = render(
        <DateRangePicker
          label="Date range"
          value={{
            start: new CalendarDateTime(2019, 2, 3),
            end: new CalendarDateTime(2019, 5, 6),
          }}
          granularity="second"
        />
      );
  ...
  describe("calendar popover", function () {
    ...
    it("should not fire onChange until both date range and time range are selected", function () {
      ...
      // selecting a date should not close the popover
      let enabledCells = cells.filter(
        (cell) => !cell.hasAttribute("aria-disabled")
      );
      ...
      expect(dialog).toBeVisible();
      expect(onChange).toHaveBeenCalledTimes(1);
      let startValue = toCalendarDateTime(today(getLocalTimeZone())).set({
        day: 1,
      });
      let endValue = toCalendarDateTime(today(getLocalTimeZone())).set({
        day: 2,
      });
      expect(onChange).toHaveBeenCalledWith({
        start: startValue,
      ...
      expect(getTextValue(startDate)).toBe(
        formatter.format(startValue.toDate(getLocalTimeZone()))
      );
      expect(getTextValue(endDate)).toBe(
  ...
  describe("labeling", function () {
    it("should support labeling", function () {
      let { getAllByRole, getByTestId, getByText } = render(
      ...
      for (let segment of startSegments) {
        ...
        expect(
          segment.getAttribute("aria-label").startsWith("Start Date ")
        ).toBe(true);
      ...
      for (let segment of endSegments) {
        expect(segment).toHaveAttribute("id");
        expect(segment.getAttribute("aria-label").startsWith("End Date ")).toBe(
          true
        );
        expect(segment).toHaveAttribute(
          "aria-labelledby",
    ...
    it("should support labeling with aria-label", function () {
      let { getAllByRole, getByTestId } = render(
        <DateRangePicker aria-label="Birth date" />
      );
    ...
    it("should support labeling with aria-labelledby", function () {
      let { getAllByRole, getByTestId } = render(
        <DateRangePicker aria-labelledby="foo" />
      );
    ...
    it("should support help text with a value", function () {
      let { getByRole, getByTestId } = render(
        <DateRangePicker
          ...
          value={{
            ...
            end: new CalendarDate(2020, 2, 10),
          }}
        />
      );
      ...
      expect(description).toBe(
        "Selected Range: February 3 to 10, 2020 Help text"
      );
    ...
    it("should support error message with a value", function () {
      ...
      let description = group
        ...
        .join(" ");
      expect(description).toBe(
        "Selected Range: February 3 to 10, 2020 Error message"
      );
    ...
    it("should have selected range description with a time", function () {
      ...
      let description = group
        ...
        .join(" ");
      expect(description).toBe(
        "Selected Range: February 3, 2020, 8:00 AM to February 10, 2020, 10:00 AM"
      );
    });
    ...
    it("should handle selected range description when start and end dates are the same", function () {
      ...
      let description = group
        ...
        .join(" ");
      expect(description).toBe(
        "Selected Range: February 3, 2020, 8:00 AM to February 3, 2020, 8:00 AM"
      );
    });
    ...
    it("should include era for BC dates", function () {
      let { getAllByRole } = render(
        <DateRangePicker
          label="Date"
          value={{
            start: new CalendarDate("BC", 1, 12, 3),
            end: new CalendarDate(1, 1, 10),
          }}
        />
      );
      ...
      let description = group
        ...
        .join(" ");
      expect(description).toBe(
        "Selected Range: December 3, 1 BC to January 10, 1 AD"
      );
    ...
    it("should display an error icon when the start date is less than the minimum (uncontrolled)", function () {
      let { getByTestId, getByLabelText } = render(
        <DateRangePicker
          label="Date range"
          defaultValue={{
            start: new CalendarDate(1985, 1, 1),
            end: new CalendarDate(1999, 2, 3),
          }}
          minValue={new CalendarDate(1985, 1, 1)}
        />
    ...
    it("should display an error icon when the start date is greater than the maximum (controlled)", function () {
      let { getByTestId } = render(
        <DateRangePicker
          label="Date range"
          value={{
            start: new CalendarDate(1990, 1, 1),
            end: new CalendarDate(1999, 2, 3),
          }}
    ...
    it("should display an error icon when the start date is greater than the maximum (uncontrolled)", function () {
      let { getByTestId, getByLabelText } = render(
        <DateRangePicker
          label="Date range"
          defaultValue={{
            start: new CalendarDate(1984, 2, 1),
            end: new CalendarDate(1984, 2, 3),
          }}
          maxValue={new CalendarDate(1985, 1, 1)}
        />
      );
    ...
    it("should display an error icon when the end date is greater than the maximum (uncontrolled)", function () {
      let { getByTestId, getByLabelText } = render(
        <DateRangePicker
          label="Date range"
          defaultValue={{
            start: new CalendarDate(1980, 2, 1),
            end: new CalendarDate(1984, 2, 3),
          }}
          maxValue={new CalendarDate(1985, 1, 1)}
        />
  ...
  describe("placeholder", function () {
    ...
    it("should reset to the placeholder if controlled value is set to null", function () {
      ...
      rerender(
        <DateRangePicker label="Date" onChange={onChange} value={null} />
      );
