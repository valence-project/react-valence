...
describe("DialogTrigger", function () {
  ...
  it("should set aria-hidden on parent providers on mount and remove on unmount", async function () {
    ...
    expect(rootProviderRef.current.UNSAFE_getDOMNode()).not.toHaveAttribute(
      "aria-hidden"
    );
    ...
    expect(rootProviderRef.current.UNSAFE_getDOMNode()).toHaveAttribute(
      "aria-hidden",
      "true"
    );
  ...
  it("can be uncontrolled with defaultOpen", async function () {
    ...
    let onOpenChange = jest.fn();
    let { getByRole } = render(
      <Test defaultOpen onOpenChange={onOpenChange} />
    );
