...
import {
  ...
  DataTransferItem,
  DragEvent,
} from "@react-aria/dnd/test/mocks";
import {
  ...
  describe("drag and drop", function () {
    ...
    describe("via mouse", function () {
      it("should show a default drag preview on drag", function () {
        ...
        fireEvent(
          cell,
          new DragEvent("dragend", { dataTransfer, clientX: 5, clientY: 5 })
        );
      ...
      it("should allow drag and drop of a single row", async function () {
        ...
        fireEvent(
          droppable,
          new DragEvent("drop", { dataTransfer, clientX: 1, clientY: 1 })
        );
        act(() => jest.runAllTimers());
        ...
        fireEvent(
          cell,
          new DragEvent("dragend", { dataTransfer, clientX: 1, clientY: 1 })
        );
      ...
      it("should allow drag and drop of multiple rows", async function () {
        ...
        expect(new Set(onSelectionChange.mock.calls[3][0])).toEqual(
          new Set(["a", "b", "c", "d"])
        );
        ...
        expect(
          await onDrop.mock.calls[0][0].items[1].getText("text/plain")
        ).toBe("Adobe XD");
        expect(
          await onDrop.mock.calls[0][0].items[2].getText("text/plain")
        ).toBe("Documents");
        expect(
          await onDrop.mock.calls[0][0].items[3].getText("text/plain")
        ).toBe("Adobe InDesign");
      ...
      it("should allow moving one item within a list", async function () {
        ...
        fireEvent(
          cell,
          new DragEvent("drag", { dataTransfer, clientX: 1, clientY: 110 })
        );
        fireEvent(
          grid,
          new DragEvent("dragover", { dataTransfer, clientX: 1, clientY: 110 })
        );
        fireEvent.pointerUp(cell, {
          pointerType: "mouse",
          button: 0,
        ...
        fireEvent(
          grid,
          new DragEvent("drop", { dataTransfer, clientX: 1, clientY: 110 })
        );
        act(() => jest.runAllTimers());
        ...
        fireEvent(
          cell,
          new DragEvent("dragend", { dataTransfer, clientX: 1, clientY: 110 })
        );
        expect(onDragEnd).toHaveBeenCalledTimes(1);
      ...
      it("should allow moving multiple items within a list", async function () {
        ...
        expect(within(rows[3]).getByRole("gridcell")).toHaveTextContent(
          "Item Four"
        );
      ...
      it("should allow moving one item into another list", async function () {
        ...
        expect(within(list1rows[0]).getByRole("gridcell")).toHaveTextContent(
          "Item One"
        );
        ...
        expect(within(list2rows[0]).getByRole("gridcell")).toHaveTextContent(
          "Item Seven"
        );
        expect(within(list2rows[1]).getByRole("gridcell")).toHaveTextContent(
          "Item Eight"
        );
        expect(within(list2rows[2]).getByRole("gridcell")).toHaveTextContent(
          "Item Nine"
        );
        ...
        fireEvent(
          list1rows[0],
          new DragEvent("drag", { dataTransfer, clientX: 0, clientY: 0 })
        );
        fireEvent(
          list1rows[0],
          new DragEvent("dragenter", { dataTransfer, clientX: 0, clientY: 0 })
        );
        fireEvent(
          grid2,
          new DragEvent("dragover", { dataTransfer, clientX: 0, clientY: 0 })
        );
        expect(onDragStart).toHaveBeenCalledTimes(1);
        ...
        fireEvent(
          list1rows[0],
          new DragEvent("drag", { dataTransfer, clientX: 200, clientY: -1 })
        );
        fireEvent(
          list1rows[0],
        ...
        fireEvent(
          grid2,
          new DragEvent("dragover", { dataTransfer, clientX: 200, clientY: -1 })
        );
        ...
        fireEvent(
          grid2,
          new DragEvent("drop", { dataTransfer, clientX: 200, clientY: -1 })
        );
    ...
    describe("via keyboard", function () {
      ...
      it("should allow drag and drop of multiple rows", async function () {
        let { getAllByRole, getByText } = render(
          <DraggableListView
            listViewProps={{ selectedKeys: ["a", "b", "c", "d"] }}
          />
        );
      ...
      it("should allow moving one item within a list", async function () {
        ...
        userEvent.tab();
        let draghandle = within(getAllByRole("row")[1]).getAllByRole(
          "button"
        )[0];
        expect(draghandle).toBeTruthy();
      ...
      it("should allow moving multiple items within a list", async function () {
        ...
        let draghandle = within(getAllByRole("row")[2]).getAllByRole(
          "button"
        )[0];
        expect(draghandle).toBeTruthy();
      ...
      it("should allow moving one item into another list", async function () {
        ...
        let draghandle = within(getAllByRole("row")[0]).getAllByRole(
          "button"
        )[0];
        expect(draghandle).toBeTruthy();
    ...
    it('should display the drag handle on hover, press, or keyboard focus for disabled/non dragggable items with disabledBehavior="selection"', function () {
      ...
      let { getAllByRole } = render(
        <DraggableListView
          listViewProps={{ disabledKeys: ["a"], disabledBehavior: "selection" }}
        />
      );
      ...
      async function ({ disabledKeys, itemLabels }) {
        ...
        for (let i = 0; i <= 6; i++) {
          if (itemLabels[i] === "Six") {
            expect(document.activeElement).toHaveAttribute(
              "aria-label",
              "Insert after Item Six"
            );
          } else if (itemLabels[i] === "before") {
            expect(document.activeElement).toHaveAttribute(
              "aria-label",
              "Insert before Item One"
            );
          } else {
            expect(document.activeElement).toHaveAttribute(
              "aria-label",
              `Insert between Item ${itemLabels[i]} and Item ${
                itemLabels[i + 1]
              }`
            );
          }
    ...
    it("should only count the selected keys that exist in the collection when dragging and dropping", async function () {
      ...
      expect(new Set(onSelectionChange.mock.calls[2][0])).toEqual(
        new Set(["1", "2", "3"])
      );
      ...
      act(() => jest.runAllTimers());
      let droppableButton = await within(droppable).findByLabelText(
        "Drop on Folder 2",
        { hidden: true }
      );
    ...
    it("should automatically focus the newly added dropped item", async function () {
      ...
      fireEvent(
        grids[1],
        new DragEvent("dragover", { dataTransfer, clientX: 1, clientY: 1 })
      );
      ...
      await waitFor(
        () => expect(within(grids[1]).getAllByRole("row")).toHaveLength(7),
        { interval: 10 }
      );
      expect(onDrop).toHaveBeenCalledTimes(1);
      ...
      fireEvent(
        grids[1],
        new DragEvent("dragover", { dataTransfer, clientX: 1, clientY: 2 })
      );
      fireEvent.pointerUp(draggedCell, {
        pointerType: "mouse",
        button: 0,
      ...
      await waitFor(
        () => expect(within(grids[1]).getAllByRole("row")).toHaveLength(8),
        { interval: 10 }
      );
    ...
    it("should support getAllowedDropOperations to limit allowed operations", () => {
      let getAllowedDropOperations = jest
        .fn()
    ...
    describe("accessibility", function () {
      it("drag handle should reflect the correct number of draggable rows", function () {
        let { getAllByRole } = render(
          <DraggableListView
            listViewProps={{ defaultSelectedKeys: ["a", "b", "c"] }}
          />
        );
        ...
        expect(dragButtonA).toHaveAttribute(
          "aria-label",
        ...
        expect(dragButtonB).toHaveAttribute(
          "aria-label",
          "Drag 3 selected items"
        );
        ...
        expect(dragButtonD).toHaveAttribute(
          "aria-label",
          "Drag Adobe InDesign"
        );
