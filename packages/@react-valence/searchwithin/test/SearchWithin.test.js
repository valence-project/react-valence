...
describe("SearchWithin", function () {
  ...
  it("should support custom icon", function () {
    let { getByTestId } = renderSearchWithin(
      {},
  ...
  it("searchfield and picker are labelled correctly", function () {
    ...
    expect(searchfield).toHaveAttribute(
      "aria-labelledby",
      `${label.id} ${hiddenLabel.id} ${picker.id}`
    );
...
describe("SearchWithin labeling", function () {
  it("no label - default", function () {
    ...
    expect(searchfield).toHaveAttribute(
      "aria-labelledby",
      `${hiddenLabel.id} ${picker.id}`
    );
    expect(picker).toHaveAttribute(
      "aria-labelledby",
      `${hiddenLabel.id} ${picker.childNodes[0].id}`
    );
  });
  ...
  it("label = foo", function () {
    ...
    expect(picker).toHaveAttribute(
      "aria-labelledby",
      `${label.id} ${hiddenLabel.id} ${picker.childNodes[0].id}`
    );
  ...
  it("aria-label = bar", function () {
    let { getByRole, getByText } = renderSearchWithin({
      "aria-label": "bar",
      label: undefined,
    });
    ...
    expect(searchfield).toHaveAttribute(
      "aria-labelledby",
      `${group.id} ${hiddenLabel.id} ${picker.id}`
    );
    expect(picker).toHaveAttribute(
      "aria-labelledby",
      `${group.id} ${hiddenLabel.id} ${picker.childNodes[0].id}`
    );
  });
  ...
  it("aria-labelledby = {id}", function () {
    ...
    expect(searchfield).toHaveAttribute(
      "aria-labelledby",
      `id-foo-label ${hiddenLabel.id} ${picker.id}`
    );
    expect(searchfield).toHaveAttribute("id", "id-searchfield");
    expect(picker).toHaveAttribute(
      "aria-labelledby",
      `id-foo-label ${hiddenLabel.id} ${picker.childNodes[0].id}`
    );
  });
});
