...
let refExists = (ComponentToCheck, children, props) => {
  ...
  let Component = () => {
    ref = useRef();
    return (
      <ComponentToCheck ref={ref} {...props} data-testid={dataTestId}>
        {children}
      </ComponentToCheck>
    );
  };
...
describe("Well", () => {
  ...
  `("$Name supports additional properties", ({ Component, props }) => {
    let { getByTestId } = render(<Component {...props}>My Well</Component>);
    expect(getByTestId(props["data-testid"])).toHaveAttribute(
      "data-testid",
      props["data-testid"]
    );
  });
  ...
  it("v3 forward ref exists and supports children and props", function () {
    let { ref } = refExists(Well, "Well Text", {
      "data-testid": "wellForwardRef",
    });
    expect(ref.current.UNSAFE_getDOMNode()).toHaveAttribute(
      "data-testid",
      "wellForwardRef"
    );
    expect(
      ref.current.UNSAFE_getDOMNode().textContent.includes("Well Text")
    ).toBeTruthy();
  });
