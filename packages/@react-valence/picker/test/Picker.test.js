...
describe("Picker", function () {
  ...
  describe("opening", function () {
    ...
    it("scrolls the selected item into view on menu open", function () {
      ...
      let virtualizerMock = jest
        .spyOn(Virtualizer.prototype, "scrollToItem")
        .mockImplementationOnce(scrollToSpy);
      // Mock scroll height so that the picker heights actually have a value
      let scrollHeightSpy = jest
  ...
  describe("autofill", function () {
    it("should have a hidden select element for form autocomplete", function () {
      ...
      options.forEach(
        (option, index) =>
          index > 0 && expect(option).toHaveTextContent(states[index - 1].name)
      );
