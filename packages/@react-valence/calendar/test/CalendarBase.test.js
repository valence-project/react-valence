...
let headingFormatter = new Intl.DateTimeFormat("en-US", {
  month: "long",
  year: "numeric",
});
let keyCodes = {
...
describe("CalendarBase", () => {
  ...
  describe("basics", () => {
    ...
    `("$Name shows the current month by default", ({ Calendar, props }) => {
      let { getByLabelText, getAllByLabelText, getByRole, getAllByRole } =
      ...
      expect(todayCell).toHaveAttribute(
        "aria-label",
        `Today, ${cellFormatter.format(new Date())}`
      );
      ...
      expect(gridCells.length).toBe(
        new GregorianCalendar().getDaysInMonth(today())
      );
    ...
    `(
      "$Name should set aria-disabled when isDisabled",
      ({ Calendar, props }) => {
        let { getByRole, getAllByRole, getByLabelText, getAllByLabelText } =
          render(<Calendar {...props} />);
    ...
    `(
      "$Name should set aria-readonly when isReadOnly",
      ({ Calendar, props }) => {
    ...
    `(
      "$Name should disable the next button if outside valid date range",
      ({ Calendar, props }) => {
        let { getByLabelText, getAllByLabelText } = render(
          <Calendar {...props} />
        );
    ...
    `(
      ...
      ({ Calendar, props }) => {
        ...
        expect(
          getAllByLabelText("selected", { exact: false }).length
        ).toBeGreaterThan(0);
      ...
      ({ Calendar, props }) => {
        ...
        expect(
          document.activeElement
            ...
            .startsWith("Sunday, February 10, 2019")
        ).toBe(true);
        ...
        expect(
          document.activeElement
            .getAttribute("aria-label")
            .startsWith("Wednesday, April 10, 2019")
        ).toBe(true);
      }
    );
    ...
    it.each`
      ...
      ${"v3 RangeCalendar"} | ${RangeCalendar}
    `(
      "$Name should handle minimum dates in a calendar system",
      ({ Calendar }) => {
    ...
    it.each`
      ...
      ${"v3 RangeCalendar"} | ${RangeCalendar}
    `(
      "$Name should handle maximum dates in a calendar system",
      ({ Calendar }) => {
        let { getByRole, getAllByRole } = render(
    ...
    `("$Name should show era for BC dates", ({ Calendar }) => {
      let { getByRole } = render(
        <Calendar defaultFocusedValue={new CalendarDate("BC", 2, 1, 5)} />
      );
    ...
    `(
      ...
      ({ Calendar, props }) => {
        ...
        expect(
          getByRole("button", { name: "Monday, December 6, 2021" })
        ).toHaveAttribute("aria-disabled", "true");
        expect(
          getByRole("button", { name: "Tuesday, December 7, 2021" })
        ).toHaveAttribute("aria-disabled", "true");
        expect(
          getByRole("button", { name: "Wednesday, December 8, 2021" })
        ).toHaveAttribute("aria-disabled", "true");
        expect(
          getByRole("button", { name: "Thursday, December 9, 2021" })
        ).toHaveAttribute("aria-disabled", "true");
        expect(
          getByRole("button", { name: "Friday, December 10, 2021" })
        ).toHaveAttribute("aria-disabled", "true");

        expect(
          getByRole("button", { name: "Wednesday, December 22, 2021" })
        ).toHaveAttribute("aria-disabled", "true");
        expect(
          getByRole("button", { name: "Thursday, December 23, 2021" })
        ).toHaveAttribute("aria-disabled", "true");
        expect(
          getByRole("button", { name: "Friday, December 24, 2021" })
        ).toHaveAttribute("aria-disabled", "true");
        expect(
          getByRole("button", { name: "Saturday, December 25, 2021" })
        ).toHaveAttribute("aria-disabled", "true");
        expect(
          getByRole("button", { name: "Sunday, December 26, 2021" })
        ).toHaveAttribute("aria-disabled", "true");
        ...
        let cell = getByRole("button", {
          name: "Wednesday, December 22, 2021",
        });
        triggerPress(cell);
    ...
    `("$Name should support defaultFocusedValue", ({ Calendar }) => {
      ...
      expect(
        document.activeElement
          .getAttribute("aria-label")
          .startsWith("Wednesday, June 5, 2019")
      ).toBe(true);
      ...
      expect(
        document.activeElement
          .getAttribute("aria-label")
          .startsWith("Thursday, June 6, 2019")
      ).toBe(true);
    ...
    `("$Name should support focusing via a ref", ({ Calendar }) => {
      ...
      expect(document.activeElement.parentElement).toHaveAttribute(
        "role",
        "gridcell"
      );
    });
    ...
    it.each`
      ...
      ${"v3 RangeCalendar"} | ${RangeCalendar}
    `(
      "$Name should return the outer most DOM element from the ref",
      ({ Calendar }) => {
        let ref = React.createRef();
        render(<Calendar ref={ref} />);
  ...
  describe("labeling", () => {
    ...
    `(
      "$Name should be labeled by month heading by default",
    ...
    it.each`
      ...
      ${"v3 RangeCalendar"} | ${RangeCalendar} | ${{ defaultValue: { start: new CalendarDate(2019, 6, 5), end: new CalendarDate(2019, 6, 5) } }}
    `(
      "$Name should support labeling with aria-label",
      ({ Calendar, props }) => {
    ...
    it.each`
      ...
      ${"v3 RangeCalendar"} | ${RangeCalendar} | ${{ defaultValue: { start: new CalendarDate(2019, 6, 5), end: new CalendarDate(2019, 6, 5) } }}
    `(
      "$Name should support labeling with aria-labelledby",
      ({ Calendar, props }) => {
        let { getByRole } = render(
          <Calendar {...props} aria-labelledby="foo" />
        );
        let calendar = getByRole("group");
        let body = getByRole("grid");
        expect(calendar).toHaveAttribute("id");
    ...
    it.each`
      ...
      ${"v3 RangeCalendar"} | ${RangeCalendar} | ${{ defaultValue: { start: new CalendarDate(2019, 6, 5), end: new CalendarDate(2019, 6, 5) } }}
    `(
      "$Name should support labeling with aria-labelledby and aria-label",
      ({ Calendar, props }) => {
        let { getByRole } = render(
          <Calendar {...props} aria-label="cal" aria-labelledby="foo" />
        );
        let calendar = getByRole("group");
        let body = getByRole("grid");
        expect(calendar).toHaveAttribute("id");
    ...
    it.each`
      ...
      ${"v3 RangeCalendar"} | ${RangeCalendar} | ${{ defaultValue: { start: new CalendarDate(2019, 6, 5), end: new CalendarDate(2019, 6, 5) } }}
    `(
      "$Name should support labeling with a custom id",
      ({ Calendar, props }) => {
        let { getByRole } = render(
    ...
    it.each`
      ...
      ${"v3 RangeCalendar"} | ${RangeCalendar} | ${{ defaultValue: { start: new CalendarDate(2019, 6, 5), end: new CalendarDate(2019, 6, 5) } }}
    `(
      "$Name should support labeling with multiple visible months",
      ({ Calendar, props }) => {
        let { getByRole, getAllByRole } = render(
  ...
  describe("internationalization", () => {
    it.each`
      ...
      ${"v3 RangeCalendar"} | ${RangeCalendar} | ${{}}
    `(
      "$Name should change the week start day based on the locale",
      ({ Calendar }) => {
    ...
    it.each`
      ...
      ${"v3 RangeCalendar"} | ${RangeCalendar} | ${{ defaultValue: { start: new CalendarDate(2019, 6, 5), end: new CalendarDate(2019, 6, 10) } }}
    `(
      "$Name should mirror arrow key movement in an RTL locale",
      ({ Calendar, props }) => {
        // LTR
        ...
        expect(document.activeElement).toBe(
          selected.parentNode.previousSibling.children[0]
        );
        ...
        fireEvent.keyDown(grid, { key: "ArrowLeft" });
        expect(document.activeElement).toBe(
          selected.parentNode.nextSibling.children[0]
        );
