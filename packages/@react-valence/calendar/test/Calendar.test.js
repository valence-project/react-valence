...
describe("Calendar", () => {
  ...
  describe("basics", () => {
    ...
    it.each`
      ...
      ${"v3"} | ${Calendar}
    `(
      "$Name should focus the selected date if autoFocus is set",
      ({ Calendar }) => {
    ...
    it("should show era for BC dates", () => {
      let { getByLabelText } = render(
        <Calendar value={new CalendarDate("BC", 5, 2, 3)} />
      );
      let cell = getByLabelText("selected", { exact: false });
  ...
  describe("selection", () => {
    it.each`
      ...
      ${"v3"} | ${Calendar}
    `(
      "$Name selects a date on keyDown Enter/Space (uncontrolled)",
      ({ Calendar }) => {
    ...
    it.each`
      ...
      ${"v3"} | ${Calendar}
    `(
      "$Name selects a date on keyDown Enter/Space (controlled)",
      ({ Calendar }) => {
        let onChange = jest.fn();
        let { getByLabelText, getByRole } = render(
          <Calendar
    ...
    it.each`
      ...
      ${"v3"} | ${Calendar} | ${{ isReadOnly: true }}
    `(
      "$Name does not select a date on click if isReadOnly",
      ({ Calendar, props }) => {
        let onChange = jest.fn();
        let { getByLabelText, getByText } = render(
          <Calendar
    ...
    `(
      ...
      ({ Calendar, props }) => {
        let onChange = jest.fn();
        let { getByLabelText } = render(
          <Calendar onChange={onChange} {...props} />
        );
  ...
  describe("announcing", () => {
    ...
    it("announces when the selected date changes", () => {
      ...
      expect(announce).toHaveBeenCalledWith(
        "Selected Date: Monday, June 17, 2019",
        "polite",
        4000
      );
    });
    ...
    it("ensures that the active descendant is announced when the focused date changes", () => {
      ...
      expect(
        getByLabelText("Thursday, June 6, 2019", { exact: false })
      ).toHaveFocus();
    });
