...
describe("RangeCalendar", () => {
  ...
  describe("basics", () => {
    ...
    `(
      "$Name should render a calendar with a defaultValue",
      ({ RangeCalendar, props }) => {
    ...
    `(
      "$Name should render a calendar with a value",
      ({ RangeCalendar, props }) => {
        let { getAllByLabelText, getByRole, getAllByRole } = render(
          <RangeCalendar {...props} />
        );
      ...
      let { getAllByRole, getAllByLabelText } = render(
        <RangeCalendar
          value={{
            ...
            end: new CalendarDate(2019, 3, 10),
          }}
          visibleMonths={3}
        />
      );
    ...
    it("should show era for BC dates", () => {
      let { getAllByLabelText } = render(
        <RangeCalendar
          value={{
            start: new CalendarDate("BC", 1, 12, 14),
            end: new CalendarDate(1, 1, 22),
          }}
        />
      );
  ...
  describe("selection", () => {
    it.each`
      ...
      ${"v3"} | ${RangeCalendar} | ${{}}
    `(
      "$Name adds a range selection prompt to the focused cell",
      ({ RangeCalendar, props }) => {
        ...
        ).toHaveTextContent("Click to start selecting date range");
        ...
        expect(
          document.getElementById(cell.getAttribute("aria-describedby"))
        ).toHaveTextContent("Click to finish selecting date range");
      }
    );
    ...
    it.each`
      ...
      ${"v3"} | ${RangeCalendar} | ${{ value: { start: new CalendarDate(2019, 6, 5), end: new CalendarDate(2019, 6, 10) } }}
    `(
      "$Name can select a range with the keyboard (controlled)",
      ({ RangeCalendar, props }) => {
        let onChange = jest.fn();
        let { getAllByLabelText } = render(
          <RangeCalendar {...props} autoFocus onChange={onChange} />
    ...
    it("does not enter selection mode with the keyboard if isReadOnly", () => {
      let { getByRole, getByLabelText } = render(
        <RangeCalendar isReadOnly autoFocus />
      );
    ...
    it.each`
      ...
      ${"v3"} | ${RangeCalendar} | ${{ defaultValue: { start: new CalendarDate(2019, 6, 5), end: new CalendarDate(2019, 6, 10) } }}
    `(
      "$Name selects a range with the mouse (uncontrolled)",
      ({ RangeCalendar, props }) => {
        let onChange = jest.fn();
        let { getAllByLabelText, getByText } = render(
          <RangeCalendar {...props} onChange={onChange} />
        );
    ...
    it.each`
      ...
      ${"v3"} | ${RangeCalendar} | ${{ value: { start: new CalendarDate(2019, 6, 5), end: new CalendarDate(2019, 6, 10) } }}
    `(
      "$Name selects a range with the mouse (controlled)",
      ({ RangeCalendar, props }) => {
        let onChange = jest.fn();
        let { getAllByLabelText, getByText } = render(
          <RangeCalendar {...props} onChange={onChange} />
    ...
    describe("touch", () => {
      ...
      it("does not allow dragging the end of an invalid range", () => {
        ...
        fireEvent.pointerUp(getByText("19"), {
          pointerType: "touch",
          clientX: 100,
          clientY: 100,
        });
    ...
    `(
      "$Name does not select a date on click if isDisabled",
      ({ RangeCalendar, props }) => {
        let onChange = jest.fn();
        let { getAllByLabelText, getByText } = render(
          <RangeCalendar {...props} onChange={onChange} />
    ...
    `(
      ...
      ({ RangeCalendar, props }) => {
        ...
        userEvent.click(
          getByLabelText("Tuesday, February 5, 2019, First available date")
        );
        ...
        userEvent.click(
          getByLabelText("Friday, February 15, 2019, Last available date")
        );
    ...
    it("disables dates not reachable from start date if isDateUnavailable is provided", () => {
      ...
      let cellBefore = getByRole("button", {
        name: "Sunday, December 5, 2021",
      });
      let cellAfter = getByRole("button", {
        name: "Monday, December 27, 2021",
      });
    ...
    it("disables the previous button if the last day of the previous month is unavailable", () => {
      const isDateUnavailable = (date) => {
        const disabledIntervals = [
          [new CalendarDate(2022, 4, 25), new CalendarDate(2022, 4, 30)],
        ];
        return disabledIntervals.some(
          (interval) =>
            date.compare(interval[0]) >= 0 && date.compare(interval[1]) <= 0
      ...
      let { getByRole, getAllByLabelText } = render(
        <RangeCalendar
          defaultValue={{
            start: new CalendarDate(2022, 5, 10),
            end: new CalendarDate(2022, 5, 12),
          }}
          isDateUnavailable={isDateUnavailable}
        />
      );
    ...
    it("disables the next button if the first day of the next month is unavailable", () => {
      const isDateUnavailable = (date) => {
        const disabledIntervals = [
          [new CalendarDate(2022, 5, 1), new CalendarDate(2022, 5, 4)],
        ];
        return disabledIntervals.some(
          (interval) =>
            date.compare(interval[0]) >= 0 && date.compare(interval[1]) <= 0
      ...
      let { getByRole, getAllByLabelText } = render(
        <RangeCalendar
          defaultValue={{
            start: new CalendarDate(2022, 4, 10),
            end: new CalendarDate(2022, 4, 12),
          }}
          isDateUnavailable={isDateUnavailable}
        />
      );
    ...
    it("updates the unavailable dates when navigating", () => {
      const isDateUnavailable = (date) => {
        const disabledIntervals = [
          [new CalendarDate(2022, 5, 2), new CalendarDate(2022, 5, 4)],
        ];
        return disabledIntervals.some(
          (interval) =>
            date.compare(interval[0]) >= 0 && date.compare(interval[1]) <= 0
      ...
      cell = getByRole("button", {
        name: "Sunday, May 1, 2022 selected, Last available date",
      });
    ...
    it("does not disable dates not reachable from start date if allowsNonContiguousRanges is provider", () => {
      ...
      expect(
        getByRole("button", { name: "Sunday, December 5, 2021" })
      ...
      expect(
        getByRole("button", { name: "Monday, December 13, 2021" })
      ).not.toHaveAttribute("aria-disabled");
    ...
    it("selects the nearest available date when blurring the calendar", () => {
      ...
      function Example() {
        ...
        return (
          <RangeCalendar
            defaultValue={{
              start: new CalendarDate(2022, 3, 1),
              end: new CalendarDate(2022, 3, 5),
            }}
            isDateUnavailable={(date) => isWeekend(date, locale)}
            allowsNonContiguousRanges
            onChange={onChange}
    ...
    it("should support validationState", () => {
      let { getByRole } = render(
        <RangeCalendar
          defaultValue={{
            start: new CalendarDate(2022, 3, 10),
            end: new CalendarDate(2022, 3, 12),
          }}
          validationState="invalid"
        />
      );
      ...
      expect(description).toBe(
        "Selected dates unavailable. Click to start selecting date range"
      );
    });
    ...
    it("automatically marks selection as invalid using isDateUnavailable", () => {
      ...
      let cell = getByRole("button", {
        name: "Friday, March 4, 2022 selected",
      });
      expect(cell).toHaveAttribute("aria-invalid", "true");
      expect(cell.parentElement).toHaveAttribute("aria-selected", "true");
      expect(cell.parentElement).toHaveAttribute("aria-invalid", "true");
