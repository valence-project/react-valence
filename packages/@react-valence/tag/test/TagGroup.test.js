...
describe("TagGroup", function () {
  ...
  it("$Name has correct tab index when disabled", () => {
    let { getAllByRole } = render(
      <TagGroup isDisabled aria-label="tag group">
        <Item aria-label="Tag 1">Tag 1</Item>
      </TagGroup>
    );
  ...
  it.each`
    Name | props | orders
    ${"(left/right arrows, ltr + horizontal) TagGroup"} | ${{ locale: "de-DE" }} | ${[{ action: () => {
      userEvent.tab();
    }, index: 0 }, { action: pressArrowRight, index: 1 }, { action: pressArrowLeft, index: 0 }, { action: pressArrowLeft, index: 2 }]}
    ${"(left/right arrows, rtl + horizontal) TagGroup"} | ${{ locale: "ar-AE" }} | ${[{ action: () => {
      userEvent.tab();
    }, index: 0 }, { action: pressArrowLeft, index: 1 }, { action: pressArrowRight, index: 0 }, { action: pressArrowRight, index: 2 }]}
    ${"(up/down arrows, ltr + horizontal) TagGroup"} | ${{ locale: "de-DE" }} | ${[{ action: () => {
      userEvent.tab();
    }, index: 0 }, { action: pressArrowDown, index: 1 }, { action: pressArrowUp, index: 0 }, { action: pressArrowUp, index: 2 }]}
    ${"(up/down arrows, rtl + horizontal) TagGroup"} | ${{ locale: "ar-AE" }} | ${[{ action: () => {
      userEvent.tab();
    }, index: 0 }, { action: pressArrowUp, index: 2 }, { action: pressArrowDown, index: 0 }, { action: pressArrowDown, index: 1 }]}
  `(
    "$Name shifts button focus in the correct direction on key press",
    function ({ Name, props, orders }) {
      let tree = render(
