...
function testSearchAutocompleteOpen(
  searchAutocomplete,
  listbox,
  focusedItemIndex
) {
  let searchAutocompleteLabelledBy =
    searchAutocomplete.getAttribute("aria-labelledby");

  expect(listbox).toBeVisible();
  expect(listbox).toHaveAttribute("aria-label", "Suggestions");
  expect(listbox).toHaveAttribute(
    "aria-labelledby",
    `${searchAutocompleteLabelledBy} ${listbox.id}`
  );
  ...
  if (typeof focusedItemIndex === "undefined") {
    ...
    act(() => {
      ...
      fireEvent.keyUp(searchAutocomplete, {
        key: "ArrowDown",
        code: 40,
        charCode: 40,
      });
...
describe("SearchAutocomplete", function () {
  ...
  it("should support custom icons", function () {
    let { getByTestId } = renderSearchAutocomplete({
      icon: <Filter data-testid="filtericon" />,
    });
  ...
  describe("controlled searchAutocomplete", function () {
    describe("controlled by inputValue", function () {
      it("updates inputValue changes", function () {
        let { getByRole, rerender } = render(
          <ExampleSearchAutocomplete inputValue="Two" />
        );
        let searchAutocomplete = getByRole("combobox");
    ...
    describe("controlled by inputValue", function () {
      it("updates the input field when inputValue prop changes", function () {
        let { getByRole, rerender } = render(
          <ExampleSearchAutocomplete inputValue="T" />
        );
        let searchAutocomplete = getByRole("combobox");
  ...
  describe("loadingState", function () {
    ...
    it('searchAutocomplete should render a loading circle if menu is not open but menuTrigger is "manual"', function () {
      ...
      expect(() =>
        within(searchAutocomplete).getByRole("progressbar")
      ).toBeTruthy();
    ...
    `(
      ...
      ({ LoadingState, ValidationState }) => {
        ...
        expect(
          within(searchAutocomplete).queryByRole("img", { hidden: true })
        ).toBeNull();
  ...
  describe("mobile searchAutocomplete", function () {
    ...
    function testSearchAutocompleteTrayOpen(
      input,
      tray,
      listbox,
      focusedItemIndex
    ...
    it("button should be labelled by external label and builtin label", function () {
      ...
      expect(button).toHaveAttribute(
        "aria-labelledby",
        `label-id ${getByText("Test").id} ${
          button.getElementsByTagName("span")[0].id
        }`
      );
    });
    ...
    it('should include invalid in label when validationState="invalid"', function () {
      let { getAllByRole, getByText, getByLabelText } =
        renderSearchAutocomplete({ validationState: "invalid" });
      let button = getAllByRole("button")[0];
      expect(button).toHaveAttribute(
        "aria-labelledby",
        `${getByText("Test").id} ${button.getElementsByTagName("span")[0].id} ${
    ...
    describe("isLoading", function () {
      ...
      `(
        ...
        ({ LoadingState, ValidationState }) => {
          let { getAllByRole, getByRole, getByTestId } =
      ...
      it('should render the loading swirl in the listbox when loadingState="loadingMore"', function () {
        let { getAllByRole, getByRole, queryByRole, getByTestId } =
          renderSearchAutocomplete({
            loadingState: "loadingMore",
            validationState: "invalid",
          });
