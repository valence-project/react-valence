...
// @types-valence
import { DOMProps, DOMRef, Node, Orientation } from "@types-valence/shared";
import { ValencePickerProps } from "@types-valence/select";
import {
  ValenceTabListProps,
  ValenceTabPanelsProps,
  ValenceTabsProps,
} from "@types-valence/tabs";

// @valence-styles
import styles from "@valence-styles/components/tabs/vars.module.scss";

interface TabsContext<T> {
  tabProps: ValenceTabsProps<T>;
  tabState: {
...
function Tabs<T extends object>(
  ...
  useEffect(() => {
    if (tablistRef.current) {
      let selectedTab: HTMLElement = tablistRef.current.querySelector(
 ...
 */
export function TabList<T>(props: ValenceTabListProps<T>) {
 ...
 */
export function TabPanels<T>(props: ValenceTabPanelsProps<T>) {
...
// @private
function TabPanel<T>(props: ValenceTabPanelsProps<T>) {
...
interface TabPickerProps<T> extends Omit<ValencePickerProps<T>, "children"> {
...
const _Tabs = React.forwardRef(Tabs) as <T>(
  props: ValenceTabsProps<T> & { ref?: DOMRef<HTMLDivElement> }
) => ReactElement;
