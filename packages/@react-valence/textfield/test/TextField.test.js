...
describe("Shared TextField behavior", () => {
  ...
  `(
    "$Name supports appending custom classnames onto the root element",
    ({ Component, props }) => {
      let tree = renderComponent(Component, props);
      expect(
        tree.container.querySelector("body>div> .custom-class-name")
      ).toBeTruthy();
    }
  );
  ...
  it.each`
    ...
    ${"v3 SearchField"} | ${SearchField}
  `(
    "$Name renders with placeholder text and shows warning",
    ({ Name, Component }) => {
      ...
      expect(spyWarn).toHaveBeenCalledWith(
        `Placeholders are deprecated due to accessibility issues. Please use help text instead. See the docs for details: https://react-spectrum.adobe.com/react-spectrum/${Name.replace(
          "v3 ",
          ""
        )}.html#help-text`
      );
  ...
  it.each`
    ...
    ${"v3 SearchField"} | ${SearchField}
  `(
    "$Name calls onFocus when the the input field is focused",
    ({ Name, Component }) => {
      let tree = renderComponent(Component, {
        onFocus,
        "aria-label": "mandatory label",
  ...
  it.each`
    ...
    ${"v3 SearchField"} | ${SearchField}
  `(
    "$Name calls onBlur when the input field loses focus",
    ({ Name, Component }) => {
      let tree = renderComponent(Component, {
        onBlur,
        autoFocus: true,
  ...
  it.each`
    ...
    ${"v3 SearchField"} | ${SearchField}
  `(
    "$Name is uncontrolled if defaultValue prop is provided",
    ({ Name, Component }) => {
  ...
  `(
    "$Name supports a isRequired prop",
    ({ Name, Component, props, expected }) => {
      let tree = renderComponent(Component, props);
      let input = tree.getByTestId(testId);
  ...
  it.each`
    ...
    ${"v3 SearchField"} | ${SearchField}
  `(
    "$Name automatically focuses the input field if autoFocus=true",
    ({ Name, Component }) => {
      let tree = renderComponent(Component, {
        autoFocus: true,
        onFocus,
  ...
  `(
    "$Name should disable the input field if isDisabled=true",
    ({ Name, Component, props }) => {
      let tree = renderComponent(Component, props);
      let input = tree.getByTestId(testId);
  ...
  it.each`
    ...
    ${"v3 SearchField"} | ${SearchField}
  `(
    "$Name will attach a ref to the input field if provided as a prop",
    ({ Component }) => {
  ...
  `("$Name supports error message", async ({ Component }) => {
    ...
    helpText = tree.getByText("Single digit numbers are 0-9.");
    expect(helpText).toHaveAttribute("id");
    expect(tree.getByTestId(testId)).toHaveAttribute(
      "aria-describedby",
      `${helpText.id}`
    );
