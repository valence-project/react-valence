...
describe("useToast", () => {
  ...
  it("handles defaults", function () {
    let { actionButtonProps, closeButtonProps, iconProps, toastProps } =
      renderToastHook({}, { onRemove });
  ...
  it("handles shouldCloseOnAction", function () {
    let { actionButtonProps } = renderToastHook(
      { onClose, onAction, shouldCloseOnAction: true },
      { onRemove }
    );
  ...
  it("onRemove is called with toastKey", function () {
    let { closeButtonProps } = renderToastHook(
      { toastKey: "key1" },
      { onRemove }
    );
    closeButtonProps.onPress();
